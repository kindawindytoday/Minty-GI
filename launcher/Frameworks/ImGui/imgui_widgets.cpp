...
bool ImGui::ArrowButton(const char* str_id, ImGuiDir dir, float sz)
{
    return ArrowButtonEx(str_id, dir, ImVec2(sz, sz), ImGuiButtonFlags_None);
}
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags)
{
    ...
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetColorU32(bg_col), ImGui::GetStyle().FrameRounding);
    if (hovered) {
        col1.x *= 0.9;
        col1.y *= 0.9;
        col1.z *= 0.9;
    ...
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(col1));
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImVec2 size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
